...
| `update_check_enabled` | `true` | Checks the WorldPumpkin GitHub Releases API for a newer version. This only reports status and never replaces files. |
...
WorldPumpkin checks its GitHub Releases API for updates. Update checks never
replace plugin files automatically.
